...
<div class="min-h-screen bg-slate-100 py-10">
    <div class="mx-auto w-full max-w-4xl px-4 sm:px-6 lg:px-8">
        <div class="rounded-3xl bg-white p-8 shadow-xl ring-1 ring-slate-200">
            ...
            <form action="{{ route('masukan.update', $masukan->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                    <div>
                        <label class="mb-2 block text-sm font-medium text-slate-700">Nama Kepala Keluarga</label>
                        <input type="text" name="nama" value="{{ old('nama', $masukan->nama) }}" class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100" required>
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium text-slate-700">Keterangan</label>
                        <input type="text" name="keterangan" value="{{ old('keterangan', $masukan->keterangan) }}" class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100" required>    
                    </div>
                    <div>
                    ...
                    @if($masukan->foto)
                        <div class="mt-3 flex items-center gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <img src="{{ asset('img/masukan/'.$masukan->foto) }}" alt="Foto Masukan" class="h-20 w-20 rounded-xl object-cover">
                            <div class="text-sm text-slate-600">Foto saat ini. Biarkan kosong jika tidak ingin mengganti.</div>
                        </div>
                    @endif
                </div>
                <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
                    <a href="{{ route('masukan.index') }}" class="inline-flex items-center justify-center rounded-2xl border border-slate-300 bg-white px-6 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50">Kembali</a>
