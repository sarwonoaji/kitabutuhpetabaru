...
@section('title', 'Tambah Data Peta Ternak Tani')
@section('content')
<div class="h-10">
    <h4 class="text-2xl text-slate-900 text-center">
        Tambah Data Peta Ternak Tani
    </h4>
</div>
<div class="min-h-screen bg-slate-100 py-10">
    <div class="mx-auto w-full max-w-4xl px-4 sm:px-6 lg:px-8">
        <div class="rounded-3xl bg-white p-8 shadow-xl ring-1 ring-slate-200">
            ...
            <form action="{{ route('ternaktani.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                    <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="deskripsi" value="{{ old('deskripsi') }}" class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100" required>
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium text-slate-700">Jumlah</label>
                        <input type="text" name="jumlah" value="{{ old('jumlah') }}" class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100" required>
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium text-slate-700">Harga</label>
                        <input type="text" name="harga" value="{{ old('harga') }}" class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100" required>
                    </div>
                    <div>
                    ...
                    <div>
                     ...
                     <div class="form-group">
                        ...
                        <input type="radio" id="ternak" name="jenis" value="ternak" {{ old('jenis') == 'ternak' ? 'checked' : '' }}> <label for="ternak">ternak</label>
                        <input type="radio" id="tani" name="jenis" value="tani" {{ old('jenis') == 'tani' ? 'checked' : '' }}> <label for="tani">tani</label>
                        <input type="radio" id="ternaktani" name="jenis" value="ternaktani" {{ old('jenis', 'ternaktani') == 'ternaktani' ? 'checked' : '' }}> <label for="ternaktani">ternaktani</label>
                    </div>
                <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
                    <a href="{{ route('ternaktani.index') }}" class="inline-flex items-center rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50">Kembali</a>
