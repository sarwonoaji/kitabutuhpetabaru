    ...
    <div class="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8">
        <div class="rounded-3xl bg-white p-8 shadow-xl ring-1 ring-slate-200">
            <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <a href="{{ route('umkm.create') }}" class="inline-flex items-center justify-center rounded-2xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white shadow-sm shadow-blue-500/20 transition hover:bg-blue-700">+ Tambah Data</a>
            </div>

            <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <form method="GET" class="w-full sm:w-auto sm:flex sm:gap-2">
                    <label for="keyword" class="sr-only">Cari umkm</label>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-slate-50">
                        ...
                        <tr class="hover:bg-slate-100">
                            ...
                            <td class="px-4 py-4 align-top">
                                <img src="{{ asset('img/umkm/'.$row->foto) }}" alt="Foto UMKM" class="h-24 w-24 rounded-2xl object-cover border border-slate-200">
                            </td>
                            <td class="px-4 py-4 align-top text-center">
                                <div class="inline-flex flex-col gap-2 sm:flex-row sm:justify-center">
                                    <a href="{{ route('umkm.edit', $row->id) }}" class="rounded-2xl bg-blue-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-blue-700">Ubah</a>
                                    <form action="{{ route('umkm.delete', $row->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data UMKM ini?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                        <tr class="bg-white">
                            <td colspan="7" class="px-4 py-8 text-center text-sm text-slate-500">Data UMKM belum tersedia.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="mt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-between">
                <div class="text-sm text-slate-500">Halaman {{ $umkm->currentPage() }} dari {{ $umkm->lastPage() }}</div>
                <div>{{ $umkm->links() }}</div>
            </div>
...
<script id="umkm-data" type="application/json">
    {!! json_encode($umkm->items(), JSON_THROW_ON_ERROR) !!}
</script>

<script>
const umkm = JSON.parse(document.getElementById('umkm-data').textContent || '[]');
...
umkm.forEach(d => {
    L.marker([d.latitude, d.longitude]).addTo(map)
        ...
        <b>Harga</b> : ${d.harga}
        <p>
        ${d.foto ? `<img src="/img/umkm/${d.foto}" width="300" style="margin-top:5px;border-radius:8px;">` : ''}
        </p>
        <br><br>
