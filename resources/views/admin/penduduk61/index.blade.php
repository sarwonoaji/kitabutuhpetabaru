...
@section('title', 'RT 6 / RW 1')
@section('content')
<div class="h-10">
    <h4 class="text-2xl text-slate-900 text-center">
        Peta Kependudukan RT 6 / RW 1
    </h4>
</div>
    ...
    <div class="mx-auto w-full max-w-6xl px-4 sm:px-6 lg:px-8">
        <div class="rounded-3xl bg-white p-8 shadow-xl ring-1 ring-slate-200">
            <div class="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <a href="{{ route('penduduk61.create') }}" class="inline-flex items-center justify-center rounded-2xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white shadow-sm shadow-blue-500/20 transition hover:bg-blue-700">+ Tambah Data</a>
            </div>

            <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <form method="GET" class="w-full sm:w-auto sm:flex sm:gap-2">
                    <label for="keyword" class="sr-only">Cari penduduk61</label>
                    <input id="keyword" type="text" name="keyword" value="{{ request('keyword') }}" placeholder="Cari penduduk61..." class="w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-2 focus:ring-blue-100">
                    ...
                    <tbody class="divide-y divide-slate-200 bg-slate-50">
                        ...
                        <tr class="hover:bg-slate-100">
                            ...
                            <td class="px-4 py-4 align-top">
                                <img src="{{ asset('img/penduduk61/'.$row->foto) }}" alt="Foto Penduduk61" class="h-24 w-24 rounded-2xl object-cover border border-slate-200">
                            </td>
                            <td class="px-4 py-4 align-top text-center">
                                <div class="inline-flex flex-col gap-2 sm:flex-row sm:justify-center">
                                    <a href="{{ route('penduduk61.edit', $row->id) }}" class="rounded-2xl bg-blue-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-blue-700">Ubah</a>
                                    <form action="{{ route('penduduk61.delete', $row->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data Penduduk61 ini?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                        <tr class="bg-white">
                            <td colspan="7" class="px-4 py-8 text-center text-sm text-slate-500">Data Penduduk61 belum tersedia.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="mt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-between">
                <div class="text-sm text-slate-500">Halaman {{ $penduduk61->currentPage() }} dari {{ $penduduk61->lastPage() }}</div>
                <div>{{ $penduduk61->links() }}</div>
            </div>
...
<script id="penduduk61-data" type="application/json">
    {!! json_encode($penduduk61->items(), JSON_THROW_ON_ERROR) !!}
</script>

<script>
const penduduk61 = JSON.parse(document.getElementById('penduduk61-data').textContent || '[]');

const map = L.map("map").setView([-7.553486, 110.632467], 16);

L.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png").addTo(map);

penduduk61.forEach(d => {
    L.marker([d.latitude, d.longitude]).addTo(map)
        ...
        <p>
        ${d.foto ? `<img src="/img/penduduk61/${d.foto}" width="300" style="margin-top:5px;border-radius:8px;">` : ''}
        </p>
        <br><br>
